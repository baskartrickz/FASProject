...
global CommitNo := "no"
global CommitYes := "commit"


class Execute_Query
{
	UserID := "sag_ahk"
	UserPW := "nuh2nKjinnFi5n5S"
	Server := "CWWAPP1429"
	;~ Database := "aeAhkProduction"
	Database := "sagahk"
	My_DB := ""
	...
	mySqlConnect()
	{
		this.My_DB := new MySQLAPI
		try
		{
			this.My_DB.Connect(this.Server,this.UserID,this.UserPW)
			this.My_DB.Select_DB(this.Database)
		}
		catch e
		{
			e := e . "MY SQL Connection Error"
			throw e 
		}
	}
	
	
	QueryTransport(SQL,commit)
	{
		MySQL_SUCCESS := 0
		StringLower, commit, commit
		Row := {}
		Field := {}
		if (this.My_DB.Query(SQL)=MySQL_SUCCESS)
		{
			if (commit = "commit")
			{
				output := this.My_DB.Affected_Rows()
				this.My_DB.Commit()
			}
			else
			{
				My_Result := this.My_DB.Store_Result()
				i := this.My_DB.Num_Rows(My_Result)
				j := this.My_DB.Field_Count()
				k := 1
				l := 1
				if (i=1)
				{
					My_Row := this.My_DB.Fetch_Row(My_Result)
					Loop, % j
					{
						a := A_Index
						b := A_PtrSize
						c := b*(a-1)
						Row[A_Index] := StrGet(NumGet(My_Row + 0, c, "UPtr"), "UTF-8")
						My_Field := this.My_DB.Fetch_Field_Direct(My_Result,A_Index-1)
						Field[A_Index] := StrGet(NumGet(My_Field + 0, 0, "UPtr"), "UTF-8")
					}
				}
				else
				{
					...
					Loop, % i
					{
						My_Row[A_Index] := this.My_DB.Fetch_Row(My_Result)
					}
					Loop, % j
					{
						My_Field := this.My_DB.Fetch_Field_Direct(My_Result,A_Index-1)
					...
					Loop, % i
					{
						Loop, % j
						{
							...
							b := A_PtrSize
							c := b*(a-1)
							Row[l] := StrGet(NumGet(My_Row[k] + 0, c, "UPtr"), "UTF-8")
							;MsgBox, % Row[l]
							l++
						}
						k++
				...
				this.My_DB.Free_Result(My_Result)
				
			}
		}
		else
		{
			err := this.My_DB.Error()
			;MsgBox Could not connect Mysql
		}
		Table := {}
		Table.Insert("Error", {Error: err})
		Table.Insert("Rows Affected", {Out: output})
		F := ""
		For Index, Value In Field
			F .= " | " . Value
		F := LTrim(F, " |")
		;MsgBox, % F
		Table.Insert("Fields", {Fields: F})
		R := ""
		For Index, Value In Row
			R .= " | " . Value
		R := LTrim(R, " | ")
		;R := RegExReplace(R, "|", "`n",, 1, InStr(R, "|",,, j))
		;MsgBox, % R
		Table.Insert("Rows", {Rows: R})
		return Table
	}
...
;===============================================================================================================================================
Obj_MySql := new Execute_Query    ; To create Object for Execute Query
;===============================================================================================================================================
	
Add_To_Queue(QueueName,Collection,Priority_:="",deferred:="",Tags:="")
{
	try{
		if (this.Obj_MySql="")
			{
				e := "MySql Object is Required"
				throw e 
			}
		
		this.Obj_MySql.mySqlConnect()

	If (QueueName = "")
	{
		e := "Queue Name is Empty"
		throw e
	}
	...
	if(Priority_ = "")
		...
		Get_Queue_ID := "select id, processid from ahkworkqueue where name='" QueueName "'"
		Data := this.Obj_MySql.QueryTransport(Get_Queue_ID,CommitNo)
		Data := Data["Rows"].Rows
		y := StrSplit(Data, "|")
		...
		loop % y.MaxIndex()
		{
			...
			If (Queue_ID = "")
			{
				e := "Invalid Queue Name"
				throw e
			}
			
				Count_ := Collection.MaxIndex()  ;Get Collection Row Count
				If ((Count_ = 0) and (Count_ = ""))
				{
					e := "No Orders Found in Collection"
					throw e
				}
				else
					{
						Get_Queue_ID := "select keyname from ahkworkqueue where id='" Queue_ID "'"
						Keyname := this.Obj_MySql.QueryTransport(Get_Queue_ID,CommitNo)
						Keyname := Keyname["Rows"].Rows                     ; Get Keyname
						;MsgBox % Keyname
						...
						{		
							v := Collection[A_Index]
							Keyvalue := v[Keyname]
							;MsgBox % Keyvalue
							x := arrayToString(v)
							if (deferred = "")
							{
								Insert_Into := "INSERT INTO ahkworkqueueitem (keyvalue,queueid,orderstatus,attempt,priority,loadedtime,lastupdated,sessionid,processid,tag,data) VALUES ('" Keyvalue "'," Queue_ID ",1,0," Priority_ ", NOW(), NOW(),1," processid ",'" Tags "','" x "')"
							}
							else
							{
						Insert_Into := "INSERT INTO ahkworkqueueitem (keyvalue,queueid,orderstatus,attempt,priority,loadedtime,deferred,lastupdated,sessionid,processid,tag,data) VALUES ('" Keyvalue "'," Queue_ID ",1, 0," Priority_ ", NOW() ," deferred ",NOW() ,1," processid ",'" Tags "','" x "')"
								;MsgBox % Insert_Into
							}
							this.Obj_MySql.QueryTransport(Insert_Into,CommitYes)
	...
	this.Obj_MySql.mySqlDisconnection()
}  ;Function


;======================================================================================================================================================================

Get_Next_Item(QueueName,key_filter:="",tag_filter:="")
{
	try
	{
		if (this.Obj_MySql="")
	...
	If (QueueName = "")
	{
		e := "Please Enter Queue Name"
		throw e
	}
		else
		{
		Get_Queue_ID := "select id from ahkworkqueue where name='" QueueName "'"
		Queue_ID := this.Obj_MySql.QueryTransport(Get_Queue_ID,CommitNo)
		Queue_ID := Queue_ID["Rows"].Rows
		;MsgBox % Queue_ID
		
			if ((key_filter != "") and (tag_filter != ""))
			{
				Next_ITEM := "SELECT id, data FROM ahkworkqueueitem WHERE orderstatus IN (1,4) AND ((deferred<=NOW()) OR deferred IS NULL) and queueid=" Queue_ID " and keyvalue = " key_filter " and tag=" tag_filter " ORDER BY loadedtime DESC , id ASC LIMIT 1"
			}
			else if ((key_filter != "") and (tag_filter = ""))
				{
					Next_ITEM := "SELECT id, data FROM ahkworkqueueitem WHERE orderstatus IN (1,4) AND ((deferred<=NOW()) OR deferred IS NULL) and queueid=" Queue_ID " and keyvalue = " key_filter " ORDER BY loadedtime DESC , id ASC LIMIT 1"
				}
				else if ((key_filter = "") and (tag_filter != ""))
					{
						Arr := StrSplit(tag_filter, ";")
						loop % Arr.MaxIndex()
						{
							ta := Arr[A_Index]
							ta := Trim(ta)
							StringLeft tagvalue, ta, 1
							StringTrimLeft ta, ta, 1
							if(tagvalue = "+")
							{
								a := a . "SELECT id, data FROM ahkworkqueueitem WHERE orderstatus IN (1,4) AND ((deferred<=NOW()) OR deferred IS NULL) and queueid=" Queue_ID " and tag LIKE '%" ta "%' and tag NOT LIKE '' ORDER BY priority DESC, loadedtime ASC , id ASC LIMIT 1 UNION"
							}
							else if(tagvalue = "-")
							{
								b := b . "SELECT id, data FROM ahkworkqueueitem WHERE orderstatus IN (1,4) AND ((deferred<=NOW()) OR deferred IS NULL) and queueid=" Queue_ID " and tag LIKE '' and tag NOT LIKE '%" ta "%' ORDER BY priority DESC, loadedtime ASC , id ASC LIMIT 1 UNION"
							}
							
						}
							StringTrimRight a, a, 5
							;MsgBox % a
							StringTrimRight b, b, 5
							;MsgBox % b
							...
							if(a = "")
							{
								StringReplace Next_ITEM, Next_ITEM, and tag LIKE '' ,,All
								StringTrimLeft Next_ITEM, Next_ITEM, 5
							}
							if(b = "")
							{
								StringReplace Next_ITEM, Next_ITEM, and tag NOT LIKE '' ,,All
								StringTrimRight Next_ITEM, Next_ITEM, 5
							}
						...
						{
							Next_ITEM := "SELECT id, data FROM ahkworkqueueitem WHERE orderstatus IN (1,4) AND ((deferred<=NOW()) OR deferred IS NULL) and queueid=" Queue_ID " ORDER BY priority DESC, deferred DESC ,id ASC LIMIT 1"
						}
							;Sleep 100
							Array := this.Obj_MySql.QueryTransport(Next_ITEM,CommitNo)
							Array_ := Array["Rows"].Rows
							;MsgBox % Array_
							y := StrSplit(Array_, "|")
		}
		;MsgBox % y.MaxIndex()
		loop % y.MaxIndex()
		{
			...
			{
				Item_ID := s
				Comp := "UPDATE ahkworkqueueitem SET orderstatus=2,lastupdated=NOW() WHERE id=" Item_ID " and orderstatus IN (1,4)"  ;To Lock the Order
				;MsgBox % Comp
				this.Obj_MySql.QueryTransport(Comp,CommitYes)
			}
				else if(A_Index = 2)
				{
					ArrayData := s
			...
			if (instr(ArrayData, "{""1"":")>0)
			{
				Collection := stringToArray(ArrayData)
			}
			else
			{
				ArrayData := "{""1"":" . ArrayData . "}"
				;MsgBox % ArrayData
				Collection := stringToArray(ArrayData)
			}
			
			Array := {}
			Array.Insert("Queue",{Item_ID:Item_ID,Collection:Collection})
...
Mark_Completed(item_id)
{
	...
	if(item_id="")
	{
		e := "Please enter ITEM_ID"
		throw e
	}
	orderstatus := this.Order_lock_Status(item_id)
	
		if(orderstatus = 2)
		{
			Comp := "UPDATE ahkworkqueueitem SET orderstatus=3,completed=NOW(),worktime=worktime+TIMESTAMPDIFF(SECOND, lastupdated, NOW()),lastupdated=NOW() WHERE id=" item_id
			;MsgBox % Comp
			this.Obj_MySql.QueryTransport(Comp,CommitYes)
		}
		else
			{
				e := "Order is not locked"
				throw e
			}
	}
	catch e
...
Mark_Exception(item_id,Exception_Reason)
{
	try
	{
	this.Obj_MySql.mySqlConnect()
	...
	if(Exception_Reason="")
	{
		e := "Exception Reason is Empty"
		throw e
	}
	orderstatus := this.Order_lock_Status(item_id)
	if (orderstatus = 2)
		{
			excep := "UPDATE ahkworkqueueitem SET orderstatus=5, exceptionreason='" Exception_Reason "',worktime=worktime+TIMESTAMPDIFF(SECOND, lastupdated, NOW()),lastupdated=NOW(),exception=NOW() WHERE id=" item_id
			;MsgBox % excep
			this.Obj_MySql.QueryTransport(excep,CommitYes)
		}
		else
			{
				e := "Order is not locked"
...
Set_Data(item_id,Collection)
{
	...
	if(orderstatus = 2)
		...
		stringForm := arrayToString(Collection)
			
		set_data := "UPDATE ahkworkqueueitem SET data='" stringForm "' WHERE id=" item_id
		this.Obj_MySql.QueryTransport(set_data,CommitYes)	
		}
		else
			{
				e := "Order is not locked"
...
Tag_Item(item_id,tag_)
{
	...
	if(orderstatus = 2)
		...
		tag := "UPDATE ahkworkqueueitem SET tag=CONCAT(tag,'" tag_ "') WHERE id=" item_id
		;MsgBox % tag
		this.Obj_MySql.QueryTransport(tag,CommitYes)
		}
		else
			{
				e := "Order is not locked"
...
Unlock_Item(item_id)
{
	...
	if(orderstatus = 2)
		{
			unlock_ := "UPDATE ahkworkqueueitem SET orderstatus=1,lastupdated=NOW() WHERE id=" item_id
			this.Obj_MySql.QueryTransport(unlock_,CommitYes)
		}
		else
			{
				e := "This order already unlocked"
				throw e
			}
...
UnTag_Item(item_id,untag_)
{
	...
	if(orderstatus = 2)
		{
		;untag := "UPDATE ahkworkqueueitem SET tag='" untag_ "' WHERE id='" item_id
		;Obj_MySql.QueryTransport(untag,commit)
		}
		else
			{
				e := "Order is not locked"
...
Update_Status(item_id,status_)
{
	...
	if(orderstatus = 2)
		{
			update := "UPDATE ahkworkqueueitem SET status='" status_ "' WHERE id=" item_id
			this.Obj_MySql.QueryTransport(update,CommitYes)
		}
		else
			{
					e := "Order is not locked"
...
Defer(item_id,DateTime_)
{
	...
	if(orderstatus = 2)
		{
			defer := "UPDATE ahkworkqueueitem SET deferred='"DateTime_ "',orderstatus=4,worktime=worktime+TIMESTAMPDIFF(SECOND, lastupdated, NOW()),lastupdated=NOW() WHERE id=" item_id
			;MsgBox % defer
			this.Obj_MySql.QueryTransport(defer,CommitYes)
		}
...
Get_Item_Data(item_id)
{
	...
	throw e
		...
		{
		Get_item := "SELECT keyvalue, priority, STATUS, attempt, loadedtime, deferred, lastupdated, exceptionreason, tag, DATA FROM ahkworkqueueitem WHERE id=" item_id
		data := this.Obj_MySql.QueryTransport(Get_item,CommitNo)
		outPut := data["Rows"].Rows
		ArrayData := StrSplit(outPut, "|")
			loop % ArrayData.MaxIndex()
			{
				x := ArrayData[A_Index]
				if(A_Index = 1) 
				{
		...
		ArrayDatas := "{""1"":" . ArrayDatas . "}"
		StringReplace ArrayDatas, ArrayDatas, %A_Space%, ,All
		;MsgBox % ArrayData
		
		
		Collection := stringToArray(ArrayDatas)
		
		Array := {}
		Array.Insert("ItemData",{keyvalue:keyvalue,priority:priority,status:status,attempt:attempt,loadedtime:loadedtime,deferred:deferred,lastupdated:lastupdated,exceptionreason:exceptionreason,tag:tag,Collection:Collection})
...
Set_Priority(item_id,priority)
{
	...
	if(item_id="")
	{
		...
		{
		set_priority := "UPDATE ahkworkqueueitem SET priority="priority "WHERE id='" item_id
		this.Obj_MySql.QueryTransport(set_priority,CommitYes)
		}
	}
	catch e
	{
...
Get_Pending_Items(QueueName)
{
	...
	If (QueueName = "")
	{
		...
		;MsgBox % Queue_ID
			...
			selectcount := "select Count(*) from ahkworkqueueitem where queueid='" Queue_ID "'and orderstatus='1'"
			data := this.Obj_MySql.QueryTransport(selectcount,CommitNo)
			Order_count := data["Rows"].Rows  
			;MsgBox % Order_count
			if (Order_count>0) 
			{
			Get_item := "select id from ahkworkqueueitem where queueid='" Queue_ID "' and orderstatus='1'" 
			data := this.Obj_MySql.QueryTransport(Get_item,CommitNo)
			id_data := data["Rows"].Rows
			;MsgBox % id_data			
			
			ArrayData := StrSplit(id_data, "|")
			Itemid_Collection := []
				loop % ArrayData.MaxIndex()
				{
					x := ArratData[A_Index]
					Itemid_Collection.Push(x)
				}
			...
			;MsgBox % Itemid_Collection.MaxIndex()
			
		Array := {}
		Array.Insert("ItemData",{RowCount:Order_count,Itemid_Collection:Itemid_Collection})
...
Is_Item_inQueue(QueueName,Keyvalue,Include_Pending,Include_Deferred,Include_Completed,Include_Exception)
{
	try
	{
		this.Obj_MySql.mySqlConnect()
	...
	If (Keyvalue = "")
	{
		...
		;MsgBox % Queue_ID
			...
			Order_count := 0
			ConcatenateQuery := ""
			selectcount := "select Count(*) from ahkworkqueueitem where queueid='" Queue_ID "'and keyvalue='" keyvalue "'"
			data := this.Obj_MySql.QueryTransport(selectcount,CommitNo)
			Order_count := data["Rows"].Rows  
			;MsgBox % Order_count
			if (Order_count>0) 
			{
				Get_item := "select Count(keyvalue) from ahkworkqueueitem where queueid='" Queue_ID "' and keyvalue='" keyvalue "'" 
				if(Include_Pending=True)
				{
					Order_count := 0
					if(ConcatenateQuery = "")
					{
						ConcatenateQuery := "orderstatus=1"
					}
					else
					{
						ConcatenateQuery := ConcatenateQuery . " OR orderstatus=1"
						;MsgBox % Get_item
					}
				}
				
				if(Include_Deferred=True)
				{
					Order_count := 0
					if(ConcatenateQuery = "")
					{
						ConcatenateQuery := "orderstatus=4"
					}
					else
					{
						ConcatenateQuery := ConcatenateQuery . " OR orderstatus=4"
						;MsgBox % Get_item
					}
				}
				...
				if(Include_Completed=True)
				{
					...
					if(ConcatenateQuery = "")
					{
						ConcatenateQuery := "orderstatus=3"
					}
					else
					{
						ConcatenateQuery := ConcatenateQuery . " OR orderstatus=3"
						;MsgBox % Get_item
					}
				}
				
				if(Include_Exception=True)
				{
					Order_count := 0
					if(ConcatenateQuery = "")
					{
						ConcatenateQuery := "orderstatus=5"
					}
					else
					{
						ConcatenateQuery := ConcatenateQuery . " OR orderstatus=5"
						;MsgBox % Get_item
					}
				}
				...
				if(ConcatenateQuery <> "")
				{	
					Get_item := Get_item . " AND (" . ConcatenateQuery . ")"
				}
...
Get_Locked_Items(QueueName,key_filter,tag_filter)
{
	try
	{
		this.Obj_MySql.mySqlConnect()
	...
	If (QueueName = "")
	{
		...
		;MsgBox % Queue_ID
			...
			if ((key_filter != "") and (tag_filter != ""))
			{
				Next_ITEM := "SELECT id, data FROM ahkworkqueueitem WHERE orderstatus=2 and queueid=" Queue_ID " and keyvalue = " key_filter " and tag=" tag_filter " ORDER BY loadedtime DESC , id ASC LIMIT 1"
			}
			else if ((key_filter != "") and (tag_filter = ""))
				{
					Next_ITEM := "SELECT id, data FROM ahkworkqueueitem WHERE orderstatus=2 and queueid=" Queue_ID " and keyvalue = " key_filter " ORDER BY loadedtime DESC , id ASC LIMIT 1"
				}
				else if ((key_filter = "") and (tag_filter != ""))
					{
						Next_ITEM := "SELECT id, data FROM ahkworkqueueitem WHERE orderstatus=2 and queueid=" Queue_ID " and tag=" tag_filter " ORDER BY priority DESC, loadedtime ASC , id ASC LIMIT 1"
					}
					else
						{
							Next_ITEM := "SELECT id, data FROM ahkworkqueueitem WHERE orderstatus=2 AND ((deferred<=NOW()) OR deferred IS NULL) and queueid=" Queue_ID " ORDER BY priority DESC, deferred DESC ,id ASC LIMIT 1"
						}
		...
		loop % y.MaxIndex()
		{
			...
			Comp := "UPDATE ahkworkqueueitem SET orderstatus=2,lastupdated=NOW() WHERE id=" Item_ID " and orderstatus=1"  ;To Lock the Order
			;MsgBox % Comp
			this.Obj_MySql.QueryTransport(Comp,CommitYes)
			ArrayData := Trim(ArrayData)
			ArrayData := "{""1"":" . ArrayData . "}"
...
Order_lock_Status(Item_ID)
{
	try
	{	
	
	Get_Queue_ID := "select orderstatus from ahkworkqueueitem where id='" Item_ID "'"
	order_status := this.Obj_MySql.QueryTransport(Get_Queue_ID,CommitNo)
	order_status := order_status["Rows"].Rows
	}
	catch e
	{
		throw e
...
/*
Order_lock_Status(Item_ID,Obj_MySql,My_DB)
{
	try
	{	
	Get_Queue_ID := "select orderstatus from ahkworkqueueitem where id='" Item_ID "'"
	order_status := Obj_MySql.QueryTransport(Get_Queue_ID,CommitNo,My_DB)
	order_status := order_status["Rows"].Rows
	return order_status
	}
	catch e
	...
	Obj_MySql.mySqlDisconnection(My_DB)
}
*/
...
Get(Credential_name)
{
	...
	{	
		...
		if(Credential_name = "")
			{
				e := "Credential Name is Empty"
				throw e
			}
	
			Data := "select exists(select * from ahkcredentials where credentialname='" Credential_name "')"
			;MsgBox % Data
			isthere := this.Obj_MySql.QueryTransport(Data,CommitNo)
			isthere := isthere["Rows"].Rows
			;MsgBox % isthere
	
				if(isthere > 0)
				{
					Data := "select username,password,expirydate,valid from ahkcredentials where credentialname='" Credential_name "'"
					;MsgBox % Data
					credentials_data := this.Obj_MySql.QueryTransport(Data,CommitNo)
					credentials_data := credentials_data["Rows"].Rows
					;MsgBox % credentials_data
					y := StrSplit(credentials_data, "|")
					;MsgBox % y.MaxIndex()
					loop % y.MaxIndex()
					{
						s := y[A_Index]
					...
					{
						e := "Credential : Credential Name is Invalid"
						throw e
					}
				
				Array := {}
				Array.Insert("Get",{Username:Username,Password:Password,date:date,valid:valid})
...
Get_Property(Credential_name,property_Name)
{
	try
	{	
		this.Obj_MySql.mySqlConnect()
		...
		if(Credential_name = "")
			{
				...
				if(property_Name = "")
					{
						e := "Property Name is Empty"
						throw e
					}
	
					Data := "select credentialid from ahkcredentials where credentialname='" Credential_name "'"
					;MsgBox % Data
					C_Id := this.Obj_MySql.QueryTransport(Data,CommitNo)
					C_Id := C_Id["Rows"].Rows
					;MsgBox % C_Id

						Data := "select propertyValue from ahkcredentialproperty where propertyName='" property_Name "' and credentialid='" C_Id "'"
						;MsgBox % Data
						credentials_data := this.Obj_MySql.QueryTransport(Data,CommitNo)
						value := credentials_data["Rows"].Rows
						;MsgBox % value
							if(value = "")
								{
...
Read_Envir_Variable(Environment_Name)
{
	try
	{
		this.Obj_MySql.mySqlConnect()
		
		Environment_Name := Trim(Environment_Name)
	
		if (Environment_Name = "")
			{
				e := "Environment : Name is Empty"
				throw e
			}
		
			Data := "select exists(select value from ahkenvironmentvar where name='" Environment_Name "')"
			value := this.Obj_MySql.QueryTransport(Data,CommitNo)
			value := value["Rows"].Rows
			;MsgBox % value
				if (value > 0)
				{
					Data := "select value from ahkenvironmentvar where name='" Environment_Name "'"
					;MsgBox % Data
					value := this.Obj_MySql.QueryTransport(Data,CommitNo)
					value := Trim(value["Rows"].Rows)
						
				}
				else
					{
						e := "Environment : Name is Invalid"
						throw e
					}	
			;return value
	}
...
Class MySQLAPI 
{
   ...
   __New(LibPath := "") {
      ...
      ; Init MySQL
      If !(MYSQL := This.Init()) {
         MsgBox, 16, MySQL Error!, Could not initialize MySQL! ; Error Area
         Return False
      }
      This.MYSQL := MYSQL
